...
- A JWT is created after successful login
- The token is stored in an HTTP-only cookie
- Protected areas are gated through `proxy.ts` and server-side session checks

### Data Model
- `User` stores profile and budgeting context such as income and savings goal
- `Expense` stores categorized expense records used for analytics and reporting

### Analytics Flow
Monthly analytics are aggregated from stored expense data.
...
Shared utility logic is used to support both:
- live authenticated user analytics
- demo analytics for guest-accessible preview pages
